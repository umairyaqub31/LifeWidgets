...
class PendingFlirts extends React.Component {
  constructor(props) {
    super(props);
    this.state = {};
    this.per_page = 10;
    ...
    if (!this.props.isActiveFetching) {
      return (
        <View style={{ justifyContent: "center", alignItems: "center" }}>
          <Text style={{ fontSize: 18 }}>No pending flirt</Text>
        </View>
      );
    }
    return null;
...
const mapStateToProps = (state) => {
  return {
    actives: state.Flirt.pendingFlirts,
    totalActive: state.Flirt.totalPending,
    isActiveFetching: state.Flirt.isPendingFetching,
  };
};
...
const mergeProps = (stateProps, dispatchProps, ownProps) => {
  ...
  return {
    ...
    fetchPendingFlirts: (per_page, page) => {
      actions.fetchPendingFlirts(dispatch, per_page, page);
    },
...
export default connect(mapStateToProps, undefined, mergeProps)(PendingFlirts);
