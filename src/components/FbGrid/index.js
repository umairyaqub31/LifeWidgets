 ...
 **/
 ...
 const renderImages = (start, overflow, images, onPress, count) => {
   return (
     <>
 ...
 export default class FbGrid extends Component {
   render() {
     const {images, style, onPress, count} = this.props;
       ...
       <View style={{...styles.container_row, ...style}}>
         ...
         {images.length > 2 && (
           <View style={styles.container}>
             {renderImages(1, false, images, onPress, count)}
           </View>
         )}
         {images.length > 3 && (
           <View style={styles.container}>
             {renderImages(3, images.length > 5, images, onPress, count)}
           </View>
         )}
       </View>
     ) : null;
