...
export class Message {
	constructor(msg = defaultMessage, currentUser) {
		console.log('121212',msg);
		console.log('13131313',store.getState().users);
		...
		this.sender_name = msg.extension ?  store.getState().users[msg.extension.sender_id].full_name : store.getState().users[msg.sender_id].full_name 

		console.log('this3433', this);
	}
	...
	static getUserName(msg, currentUser) {
		if (msg?.read_ids?.find(_id => _id !== currentUser)) {
			return STATUS_READ
		}
		if (msg?.delivered_ids?.find(msg => msg.delivered_ids !== currentUser)) {
